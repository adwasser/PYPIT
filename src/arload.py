...
def set_params_wtype(tvalue, svalue, lines="", setstr="", argnum=3):
    ...
    tvalue: dictionary
    svalue: key
    lines: str, entire input line
    setstr: str, passes the file name for error output
    argnum: int, unknown
    ...
    try:
        if type(tvalue) is int:
            ...
            if svalue.lower() == 'none':
                tvalue = None
            elif svalue[0] == '[' and svalue[-1] == ']' and ',' in svalue and len(svalue.split(':')) == 3:
                tvalue = load_sections(svalue, strtxt=setstr)
            else:
                tvalue = svalue
        ...
        # If it was None, it may not be anymore. We now need to find the new type            
        elif tvalue is None: 
...
def set_params(lines, indict, setstr=""):
    ...
    Returns
    -------
    indict: dict, with updates from lines
    """
    for i in range(len(lines)):
        ...
        if linspl[0] == 'check':
            ...
            if linspl[1] in ['check', 'match', 'combsame']:
                ...
                indict[linspl[0]][linspl[1]] = set_params_wtype(indict[linspl[0]][linspl[1]], linspl[2],
                                                                lines=tline, setstr=setstr)
            else:
                debugger.set_trace()
            ...
            if linspl[1][:6] == 'ampsec': # Amplifier Sections
                ...
                indict[linspl[0]][didx][linspl[1]] = set_params_wtype(indict[linspl[0]][didx][linspl[1]], linspl[2],
                                                                      lines=tline,setstr=setstr)
        else:
...
def load_spect(progname, specname, spect=None, lines=None):
    ...
    def initialise():
        ...
        ddet = dict({'xgap': 0.0, 'ygap': 0.0, 'ysize': 1.0, 'darkcurr': 0.0, 'ronoise': [1.0], 'gain': [1.0],
                     'saturation': 65536.0, 'nonlinear': 1.0, 'numamplifiers': 1, 'suffix': ""})
        #
        chk = dict({})
        stf = dict({'science': [], 'standard': [], 'bias': [], 'pixflat': [], 'blzflat': [],
                    'arc': [], 'trace': [], 'dark': [], 'readnoise': []})
        kyw = dict({'target': '01.OBJECT', 'idname': '01.OBSTYPE', 'time': '01.MJD', 'date': '', 'equinox': '',
                    'ra': '', 'dec': '', 'airmass': '', 'naxis0': '01.NAXIS2', 'naxis1': '01.NAXIS1',
                    'exptime': '01.EXPTIME', 'hatch': '01.TRAPDOOR', 'filter1': '01.FILTNAME',
                    'filter2': None, 'lamps': '01.LAMPNAME', 'decker': '01.DECKNAME', 'slitwid': '01.SLITWIDTH',
                    'slitlen': '01.SLITLENGTH', 'detrot': '01.DETECTORROTATION', 'cdangle': '01.XDISPANGLE',
                    'echangle': '01.ECHELLEANGLE', 'crossdisp': '01.XDISPERS', 'dichroic': '', 'disperser': '',
                    'binning': ''})
        fts = dict({'numhead': 1, 'numlamps':1, 'dataext':0, 'calwin':12.0, 'timeunit':'mjd'})
        sci = dict({'index': [], 'check': dict({}), 'idname': 'OBJECT', 'canbe': None})
        std = dict({'index': [], 'check': dict({}), 'match': dict({}), 'number': 1,
                    'idname': 'OBJECT', 'canbe': None, 'combsame': dict({})})
        pfl = dict({'index': [], 'check': dict({}), 'match': dict({}), 'number': 5,
                    'idname': 'OBJECT', 'canbe': None, 'combsame': dict({}), 'lscomb': False})
        bfl = dict({'index': [], 'check': dict({}), 'match': dict({}), 'number': 5,
                    'idname': 'OBJECT', 'canbe': None, 'combsame': dict({}), 'lscomb': False})
        trc = dict({'index': [], 'check': dict({}), 'match': dict({}), 'number': 1,
                    'idname': 'OBJECT', 'canbe': None, 'combsame': dict({}), 'lscomb': False})
        arc = dict({'index': [], 'check': dict({}), 'match': dict({}), 'number': 1,
                    'idname': 'OBJECT', 'canbe': None, 'combsame': dict({}), 'lscomb': False})
        bia = dict({'index': [], 'check': dict({}), 'match': dict({}), 'number': 5,
                    'idname': 'OBJECT', 'canbe': None, 'combsame': dict({}) })
        rn = dict({'index': [], 'check': dict({}), 'match': dict({}), 'number': 1,
                   'idname': 'OBJECT', 'canbe': None, 'combsame': dict({}) })
        drk = dict({'index': [], 'check': dict({}), 'match': dict({}), 'number': 5,
                    'idname': 'OBJECT', 'canbe': None, 'combsame': dict({}) })
        spectt = dict({'mosaic': msc, 'det': ddet, 'check': chk, 'set': stf, 'keyword': kyw, 'fits': fts, 'science': sci,
                       'standard': std, 'pixflat': pfl, 'blzflat': bfl, 'trace': trc, 'arc': arc, 'bias': bia, 'dark': drk,
                       'readnoise': rn})
        return spectt
    if lines is None:
        # Read in the default settings
